...
void PQCLEAN_MLKEM512_CLEAN_ntt(int16_t r[256]) {
    ...
    k = 1;                                      // zetas 인덱스 (0번은 안 씀)
    for (len = 128; len >= 2; len >>= 1) {      // 라운드 : 128 -> 64 -> 32 -> 16 -> 8-> 4-> 2
        for (start = 0; start < 256; start = j + len) {     // 그룹 순회
            zeta = PQCLEAN_MLKEM512_CLEAN_zetas[k++];       // 이 그룹의 제타
            for (j = start; j < start + len; j++) {         // 버터플라이 쌍
