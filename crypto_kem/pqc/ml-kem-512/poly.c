...
void PQCLEAN_MLKEM512_CLEAN_poly_frommsg(poly *r, const uint8_t msg[KYBER_INDCPA_MSGBYTES]) {
    ...
    for (i = 0; i < KYBER_N / 8; i++) {     // i = 0 ~ 31 (32바이트)
        for (j = 0; j < 8; j++) {           // j = 0 ~ 7 (8비트)
            r->coeffs[8 * i + j] = 0;       // 먼저 0으로 초기화

                                              // 대상 주소,              // 값 : 1665           , 조건 : 해당 비트가 1이면
...
void PQCLEAN_MLKEM512_CLEAN_poly_tomsg(uint8_t msg[KYBER_INDCPA_MSGBYTES], const poly *a) {
    ...
    for (i = 0; i < KYBER_N / 8; i++) {
        ...
        for (j = 0; j < 8; j++) {
            ...
            t <<= 1;    // 2t
            t += 1665;  // 2t + q/2(1665)
            t *= 80635;  // (나눗셈을 곱셈으로 대체) t = (2t + 1665) * 80635 / 2^28
            t >>= 28;   //  ≈ (2t + 1665) / 3329
            t &= 1;  // t = result mod 2
            msg[i] |= t << j; // j 번째 비트 위치에 저장장
        }
    }
}
