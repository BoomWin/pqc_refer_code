...
// 상수 정의 - api.h 에서 가져옴
#define PK_BYTES PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES   
#define SK_BYTES PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES   
#define SIG_BYTES PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_CRYPTO_BYTES           
#define SEED_BYTES PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_CRYPTO_SEEDBYTES     
...
int do_keygen(void) {
    ...
    if (PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_crypto_sign_keypair(pk, sk) != 0) {
        print_error("키 생성 실패");
        return 1;
    }
...
int do_keygen_derand(const char *seed_hex) {
    ...
    if (PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_crypto_sign_seed_keypair(pk, sk, seed) != 0) {
...
int do_sign(const char *sk_hex, const char *msg_hex) {
    ...
    if (PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_crypto_sign_signature(sig, &siglen, msg, msg_len, sk) != 0) {
        print_error("SIGN 실패");
        free(msg);
        return 1;
    }
...
int do_verify(const char *pk_hex, const char *msg_hex, const char *sig_hex) {
    ...
    int result = PQCLEAN_SPHINCSSHAKE256FSIMPLE_CLEAN_crypto_sign_verify(sig, SIG_BYTES, msg, msg_len, pk);
