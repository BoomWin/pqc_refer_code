...
// SMAUG-T5 좀 상이한점이 이 함수 원래 void로 사용되고 있었음 ??
// 사유는 조사를 해봐야할 것 같은데 이유가 있어서 그렇게 한건지 아니면 수정 사항이였는지는 모르겠으나
// int 로 변경해줬음 동작하게 하기위해서.!
...
/*=====================================================================
* KEYGEN DERAND: 고정 시드로 키 생성
* coins: 32 bytes
*======================================================================*/
int crypto_kem_keypair_derand(uint8_t *pk, uint8_t *sk, const uint8_t *coins) {
       indcpa_keypair(pk, sk);
       for (int i = 0; i < T_BYTES; i++) {
              sk[PKE_SECRETKEY_BYTES + i] = coins[i];
       }
       return 0;
}
...
/*=====================================================================
* ENCAP DERAND: 고정 시드로 캡슐화
* coins: 32 bytes (DELTA_BYTES)
*======================================================================*/
