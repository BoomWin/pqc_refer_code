...
int do_sign(const char *sk_hex, const char *msg_hex) {
    ...
    if (PQCLEAN_MLDSA44_CLEAN_crypto_sign_signature(sig, &siglen, msg, msg_len, sk) != 0) {
        print_error("SIGN 실패");
        free(msg);
        return 1;
    }
...
int do_verify(const char *pk_hex, const char *msg_hex, const char *sig_hex) {
    ...
    int result = PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify(sig, SIG_BYTES, msg, msg_len, pk);
