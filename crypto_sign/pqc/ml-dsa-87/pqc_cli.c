/**
 ML-DSA-44/65/87 CLI Wrapper - Node.js 적용하기 위함
...
// 상수 정의 - api.h 에서 가져옴
#define PK_BYTES PQCLEAN_MLDSA87_CLEAN_CRYPTO_PUBLICKEYBYTES   
#define SK_BYTES PQCLEAN_MLDSA87_CLEAN_CRYPTO_SECRETKEYBYTES 
#define SIG_BYTES PQCLEAN_MLDSA87_CLEAN_CRYPTO_BYTES           
...
int do_sign(const char *sk_hex, const char *msg_hex) {
    ...
    // Detached 서명 (ㄱ본이 : ctx 없이)
    if (PQCLEAN_MLDSA87_CLEAN_crypto_sign_signature(sig, &siglen, msg, msg_len, sk) != 0) {
        print_error("SIGN 실패");
        free(msg);
        return 1;
    }
...
int do_verify(const char *pk_hex, const char *msg_hex, const char *sig_hex) {
    ...
    int result = PQCLEAN_MLDSA87_CLEAN_crypto_sign_verify(sig, SIG_BYTES, msg, msg_len, pk);
